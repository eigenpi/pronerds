    ...
    PICTURE_TYPE pic_on_screen_val) 
{
    ...
    if ( _pic_on_screen != pic_on_screen_val) {
        if ( pic_on_screen_val == NODES && _pic_on_screen == NO_PICTURE) {
            create_button( "Window", "Toggle Ties", TOGGLE_TIES);
        }
    }
...
void GUI_GRAPHICS::redraw_screen (void) 
{
    ...
    if ( _pic_on_screen == NODES) {

        // (a) get the nodes placement coords and type;
        long nodes_count = _pronerds->nerds_graph()->nodes_count();
        for ( long n_i = 0; n_i < nodes_count; n_i++) {
            NERDS_NODE *nerds_node = _pronerds->nerds_graph()->get_node( n_i);
            _x_coords[ n_i] = nerds_node->x_coord();
            _y_coords[ n_i] = nerds_node->y_coord();
            if ( nerds_node->type() == FEEDER) {
                _node_color[ n_i] = CYAN;
            } else {
                _node_color[ n_i] = LIGHTGREY;
            }
            _node_voltage[ n_i] = nerds_node->v_k();
        }

        // (b) get the arcs;
        long arcs_count = _pronerds->nerds_graph()->arcs_count();
        for ( long a_i = 0; a_i < arcs_count; a_i++) {
            NERDS_ARC *arc = _pronerds->nerds_graph()->get_arc( a_i);
            NERDS_NODE *src_node = _pronerds->nerds_graph()->get_node( arc->src_id());
            NERDS_NODE *des_node = _pronerds->nerds_graph()->get_node( arc->des_id());
            _x_src[ a_i] = src_node->x_coord();
            _y_src[ a_i] = src_node->y_coord();
            _x_des[ a_i] = des_node->x_coord();
            _y_des[ a_i] = des_node->y_coord();

            if ( arc->type() == NOT_SECTIONALIZING) {
                _arc_color[ a_i] = BLACK;
                _arc_line[ a_i] = SOLID;
            } else if ( arc->type() == SECTIONALIZING) {
                _arc_color[ a_i] = RED;
                _arc_line[ a_i] = SOLID;
            } else {
                _arc_color[ a_i] = RED;
                _arc_line[ a_i] = DASHED;
            }
        }

        // (c) do the drawing with the updated occupancies;
        draw_nodes();
        if ( _show_ties) {
            draw_ties();
        }
...
void GUI_GRAPHICS::toggle_ties(void) 
{
    // Enables/disables drawing of ties when a the user clicks on a button.

    _show_ties = !_show_ties;
...
void GUI_GRAPHICS::alloc_draw_structs (void) 
{
    ...
    long nodes_count = _pronerds->nerds_graph()->nodes_count();
    long arcs_count = _pronerds->nerds_graph()->arcs_count();
    long nx = _pronerds->nerds_graph()->nx(); // max x coordinate for placement of nodes;
    long ny = _pronerds->nerds_graph()->ny();
    
    _x_tile_left.resize( nx);
    _y_tile_bottom.resize( ny);

    _x_coords.resize( nodes_count);
    _y_coords.resize( nodes_count);
    _node_color.resize( nodes_count); // reflect the node type;
    _node_voltage.resize( nodes_count);

    _x_src.resize( arcs_count);
    _y_src.resize( arcs_count);
    _x_des.resize( arcs_count);
    _y_des.resize( arcs_count);
    _arc_color.resize( arcs_count); // reflect the arc type;
    _arc_line.resize( arcs_count); // dashed for tie switches;

    deselect_all();
}
void GUI_GRAPHICS::deselect_all (void) 
{
    // sets the color of all arcs to default;
    long arcs_count = _pronerds->nerds_graph()->arcs_count();
    for ( long i = 0; i < arcs_count; i++) {
        _arc_color[i] = BLACK;
    }
}
    ...
    float channel_width_val)
{
    ...
    if ( !_is_gui_usable)
        return;
    long i = 0;
    long nx = _pronerds->nerds_graph()->nx();
    long ny = _pronerds->nerds_graph()->ny();
    // set now numerical values of interest for dimensions;
...
void GUI_GRAPHICS::draw_nodes (void) 
{
    // draw nodes (using the tiles' regularity tiles) here;
    ...
    // () nodes inside tiles show;
    long nodes_count = _pronerds->nerds_graph()->nodes_count();
    for ( long n_i = 0; n_i < nodes_count; n_i++) {
        y1 = _y_tile_bottom[ _y_coords[ n_i]];
        y2 = y1 + _tile_width;
        x1 = _x_tile_left[ _x_coords[ n_i]];
        x2 = x1 + _tile_width;

        setcolor ( _node_color[ n_i]);
        fillrect ( x1,y1,x2,y2);
        setcolor ( BLACK);
        drawrect ( x1,y1,x2,y2);
        setcolor ( BLACK);
        sprintf ( buf, "%d, Vk=%.2f", n_i, _node_voltage[ n_i]);
        drawtext ( (x1 + x2)/2., (y1 + y2)/2., buf, _tile_width);
    }

    // () draw all not_sectionalizing and sectionalizing branches;
    long arcs_count = _pronerds->nerds_graph()->arcs_count();
    for ( long a_i = 0; a_i < arcs_count; a_i++) {
        if ( _arc_line[ a_i] != DASHED) {
            x1 = _x_tile_left[ _x_src[ a_i]];
            y1 = _y_tile_bottom[ _y_src[ a_i]];
            x2 = _x_tile_left[ _x_des[ a_i]];
            y2 = _y_tile_bottom[ _y_des[ a_i]];

            setcolor ( _arc_color[ a_i]);
...
void GUI_GRAPHICS::draw_ties(void) 
{
    // draw the tie switches only;
    float x1=0, y1=0, x2=0, y2=0;
...
GUI_GRAPHICS::GUI_GRAPHICS( PRONERDS *pronerds)
{
